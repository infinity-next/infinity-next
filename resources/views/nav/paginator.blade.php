<nav class="pagination pagination-automatic">
	...
	<div class="pagination-buttons buttons-before">
		@if ($paginator->currentPage() > 1)
			<a class="button pagination-button pagination-first" href="{{ $paginator->url(1) }}" title="@lang('board.first')" data-instant>&lt;&lt;</a>
			<a class="button pagination-button pagination-prev" href="{{ $paginator->previousPageUrl() }}" title="@lang('board.previous')" data-instant>&lt;</a>
		@else
	...
	<div class="pagination-buttons buttons-after">
		@if ($paginator->hasMorePages())
			<a class="button pagination-button pagination-next" href="{{ $paginator->nextPageUrl() }}" title="@lang('board.next')" data-instant>&gt;</a>
			<a class="button pagination-button pagination-last" href="{{ $paginator->url($paginator->count()) }}" title="@lang('board.last')">&gt;&gt;</a>
		@else
