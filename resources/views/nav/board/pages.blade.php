<nav class="pagination @if($showPages) pagination-full @endif @if($showCatalog) pagination-catalog @endif @if($showIndex) pagination-index @endif">
	<div class="pagination-buttons buttons-pages">
		@if ($showIndex)
		<a class="button pagination-button" href="/{{ $board->board_uri }}" data-instant>Index</a>
		@endif
		
		@if ($showCatalog)
		<a class="button pagination-button" href="/{{ $board->board_uri }}/catalog" data-instant>Catalog</a>
		@endif
	...
	<div class="pagination-buttons buttons-before">
		@if ($page > 1)
			<a class="button pagination-button pagination-first" href="{{{url($board->board_uri)}}}" title="@lang('board.first')" data-instant>&lt;&lt;</a>
		@else
			<button class="pagination-button pagination-first" title="@lang('board.first')" disabled>&lt;&lt;</button>
		@endif
		
		@if ($pagePrev !== false)
			<a class="button pagination-button pagination-prev" href="{{{url("{$board->board_uri}/{$pagePrev}")}}}" title="@lang('board.previous')" data-instant>&lt;</a>
		@else
	...
	<ul class="pagination-pages">
		...
		<li class="pagination-page">
			<a class="pagination-link @if ($i == $page) pagination-active @endif" href="{{{url("{$board->board_uri}/{$i}")}}}" data-instant>{{{$i}}}</a>
		</li>
		@endfor
	</ul>
	...
	<div class="pagination-buttons buttons-after">
		@if ($pageNext !== false)
			<a class="button pagination-button pagination-next" href="{{{url("{$board->board_uri}/{$pageNext}")}}}" title="@lang('board.next')" data-instant>&gt;</a>
		@else
