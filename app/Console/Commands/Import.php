...
class Import extends Command
{
            ...
            foreach ($boards as $board) {
                ...
                if (is_readable($bannersPath)) {
                    ...
                    foreach ($banners as $bannerName) {
                        ...
                        if ($banner->isReadable() && (bool) FileFacade::get($banner)) {
                    ...
                    switch ($configName) {
                        ...
                        case 'image_reject_repost_in_thread':
                            ...
                            if (isset($config['image_reject_repost']) && $config['image_reject_repost']) {
                                $optionValue = 'board';
                            } elseif (isset($config['image_reject_repost_in_thread']) && $config['image_reject_repost_in_thread']) {
                                $optionValue = 'thread';
                            } else {
                                $optionValue = '';
                            }
    ...
    public function importInfinityPost($post, Board &$board, &$threads)
    {
        ...
        if (!$post->thread) {
            $thread = null;
        } elseif (isset($threads[$post->thread])) {
        ...
        $tBoardsTable->chunk(100, function ($boards) use (&$boardsImported, $userAdmin, $userBoardRelationships) {
            ...
            foreach ($boards as $tBoard) {
                ...
                $hBoard = new Board([
                    ...
                    'is_indexed' => (bool) $tBoard->indexed,
                    'is_overboard' => (bool) $tBoard->indexed,
                    'is_worksafe' => (bool) $tBoard->sfw,
                ]);
