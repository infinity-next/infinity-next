...
class Role extends Model
{
    ...
    public static function getAnonymousRoleForBoard($board)
    {
        ...
        if (!$role->exists) {
            $role->fill([
                ...
                'system' => false,
                'weight' => self::WEIGHT_ANONYMOUS + 5,
            ]);
            $role->save();
        }
    ...
    public static function getUnaccountableRoleForBoard($board)
    {
        ...
        $role = static::firstOrNew([
            ...
            'board_uri' => $board_uri,
            'caste' => null,
            'inherit_id' => self::ID_UNACCOUNTABLE,
        ]);
        ...
        if (!$role->exists) {
            $role->fill([
                ...
                'system' => false,
                'weight' => self::WEIGHT_UNACCOUNTABLE + 5,
            ]);
            $role->save();
        }
    ...
    public function getPermission(Permission $permission)
    {
        foreach ($this->permissions as $thisPermission) {
            if ($thisPermission->permission_id == $permission->permission_id) {
                return (bool) $thisPermission->pivot->value;
            }
        }
     ...
     *
     * @param array|int $roleIDs Role primary keys to compile together.
     *
     * @return array
     */
