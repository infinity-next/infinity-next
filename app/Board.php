...
class Board extends Model
{
    ...
    public function getStatsPlhAttribute()
    {
        ...
        $stats = $this->stats
            ...
            ->filter(function ($item) use ($timestamp) {
                return $item->stats_time->gte($timestamp);
            });
    ...
    public function getStatsPphAttribute()
    {
        ...
        if ($stats->count()) {
            ...
            return number_format($stats->sum('counter') / max($oldest->stats_time->diffInHours(), 1), 1);
        }
    ...
    public function getStatsPpdAttribute()
    {
        ...
        if ($stats->count()) {
            $oldest = $stats->sortBy('stats_time')->first();
            return number_format($stats->sum('counter') / max($oldest->stats_time->diffInDays(), 1), 0);
        }
